    ...
    IRefreshTokenRepository _refTokRepo) : IAuthService
{
    ...
    public async Task<long> SignUpUserAsync(UserCreateDto userCreateDto)
    {
        ...
        var isEmailExists = await _userRepo.GetByEmailAsync(userCreateDto.Email);
        ...
        if (isEmailExists == null)
        {
            ...
            var user = new User()
            {
                ...
                PasswordSalt = tupleFromHasher.Salt,
                RoleId = await _roleRepo.GetRoleIdByNameAsync("User")
            };
        ...
        else if (isEmailExists.Confirmer!.IsConfirmed is false)
        {
            ...
            isEmailExists.PasswordSalt = tupleFromHasher.Salt;
            isEmailExists.RoleId = await _roleRepo.GetRoleIdByNameAsync("User");
    ...
    public async Task<LoginResponseDto> LoginUserAsync(UserLoginDto userLoginDto)
    {
        ...
        var checkUserPassword = PasswordHasher.Verify(userLoginDto.Password, user.PasswordHash, user.PasswordSalt);
        ...
        var loginResponseDto = new LoginResponseDto()
        {
            ...
            User = new UserGetDto { Email = user.Confirmer.Email, ProfileImgUrl = user.ProfileImgUrl, Role = user.Role.Name, Id = user.Id, UserName = user.UserName },
            TokenType = "Bearer",
            Expires = 24
        };
    ...
    public async Task<LoginResponseDto> RefreshTokenAsync(RefreshRequestDto request)
    {
        ...
        var refreshToken = await _refTokRepo.GetRefreshTokenAsync(request.RefreshToken, userId);
    ...
    public async Task LogOut(string token) => await _refTokRepo.RemoveRefreshTokenAsync(token);
